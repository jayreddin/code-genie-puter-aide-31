...
const ChatHeader = ({
  ...
  const getFilteredModels = () => {
    let filteredModels = {...MODEL_PROVIDERS};
    
    if (settings.streamEnabled) {
      // Filter to only show stream compatible models
      Object.keys(filteredModels).forEach(provider => {
        filteredModels[provider] = filteredModels[provider].filter(model => 
          STREAM_COMPATIBLE_MODELS.includes(model)
        );
      });
    }
    ...
    if (settings.functionCallingEnabled) {
      ...
      Object.keys(filteredModels).forEach(provider => {
        filteredModels[provider] = filteredModels[provider].filter(model => 
          FUNCTION_CALLING_MODELS.includes(model)
        );
      });
    }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Button 
            ...
            className={`${isTxtImgMode ? 'bg-green-600 hover:bg-green-700 border-2 border-green-500 animate-pulse' : ''}`}
            size="sm"
          >
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="min-w-[200px] justify-center" size="sm">
                ...
                {settings.streamEnabled && (
                  <Badge variant="outline" className="ml-2 text-xs">Stream</Badge>
                )}
                ...
                <React.Fragment key={provider}>
                  ...
                  <DropdownMenuGroup>
                    {filteredModels[provider].map(model => (
                      <DropdownMenuItem 
                        key={model}
                        className={`text-center justify-center ${selectedModel === model ? 'bg-blue-50 dark:bg-blue-900/20 font-medium' : ''}`}
                  ...
                  {Object.keys(filteredModels).indexOf(provider) < Object.keys(filteredModels).length - 1 && (
                    <DropdownMenuSeparator />
                  )}
                </React.Fragment>
              ))}
        ...
        <div className="flex flex-col items-end">
          {user && (
            <div className="text-sm border border-gray-300 rounded-lg px-3 py-1 mb-2">
              {user.username}
            </div>
          )}
